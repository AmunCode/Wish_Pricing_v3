...
def main():
    ...
    if auth_ver_response.ok:
        ...
        for sku_file in glob.glob(FILE_PATH + "e20Wireless_Price*.csv"):
            ...
            shutil.move(sku_file, DESTINATION_PATH)  # moves SKU file to a different folder once data is loaded
        ...
        with open(LOGS_PATH + "error_log.txt", "a") as error_file:
            error_file.write(f"Not authorized: token invalid {time.ctime()} {current_file.split('/')[-1][6:]} \n")
